...
void init()
{
	...
	retardedHornedMonkey = loadObject("RetardedHornedMonkey.obj");
...
void handleInput(SDL_Event *mainEvent)
{
	if(mainEvent->type == SDL_KEYDOWN)
	{
		...
		case Select:
			if(SDL_GetTicks() - lastButtonTime >= _buttonspeed)
			{
				...
				printf("Select/Unselect\n");
			}
			break;
		case PanLeft:
			if(!select) cameraPos.increment(_cameraSpeed,1.0,0.0,0.0);
			break;
		case PanRight:
			if(!select) cameraPos.increment(_cameraSpeed,-1.0,0.0,0.0);
			break;
		case PanUp:
			if(!select) cameraPos.increment(_cameraSpeed,0.0,-1.0,0.0);
			break;
		case PanDown:
			if(!select) cameraPos.increment(_cameraSpeed,0.0,1.0,0.0);
			break;
		case ZoomIn:
			if(!select) cameraPos.increment(_cameraSpeed,0.0,0.0,1.0);
			break;
		case ZoomOut:
			if(!select) cameraPos.increment(_cameraSpeed,0.0,0.0,-1.0);
			break;
		case ResetCamera:
			if(!select) cameraPos.set(0.0f,0.0f,0.0f); printf("Pressed\n");
			break;
		default:
			break;
		}
	...
	if(select)
	{
		...
		if(state[_FieldForward])
		{
			if(SDL_GetTicks() - lastButtonTime > _buttonspeed)
			{
				...
				lastButtonTime = SDL_GetTicks();
				if(fieldIndex < fields.size()) {printf("Current electric field detected: "); fields[fieldIndex]->field.print();}
			}
		}
		if(state[_FieldBackward])
		{
			if(SDL_GetTicks() - lastButtonTime > _buttonspeed)
			{
				...
				lastButtonTime = SDL_GetTicks();
				if(fields.size() > 0) {printf("Current electric field detected: "); fields[fieldIndex]->field.print();}
			}
		}
